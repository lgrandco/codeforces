...
def solve():
    *_, n, x, y = map(int, input().split())
    ...
    if not between:
        ...
        return
    # debug(a, x, y)
    # debug(before, between, after)
    _min = min(between)
    while between[0] != _min:
        between.append(between.popleft())
    above = sum(e > _min for e in before)
    # debug(above)
    while above:
        if before[-1] > _min:
            above -= 1
        after.appendleft(before.pop())
    while after and after[0] < _min:
        before.append(after.popleft())
    p(*before, *between, *after)
    # a = input()
