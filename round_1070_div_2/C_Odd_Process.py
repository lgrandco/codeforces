...
def solve():
    ...
    if len(first_input.split()) > 1:
        ...
        for _ in range(
            ...
            sorted_even = sorted([e for e in a if e % 2 == 0])[::-1]
            sorted_odd = sorted([e for e in a if e % 2 == 1])[::-1]
            # debug(sorted_even, sorted_odd)
            len_even = len(sorted_even)
            len_odd = len(sorted_odd)
            even_pre = [0] * (len_even + 1)
            for i in range(len_even):
                even_pre[i + 1] = even_pre[i] + sorted_even[i]
            max_odd = sorted_odd[0] if len_odd > 0 else 0
            r = []
            for i in range(1, n + 1):
                if len_odd == 0:
                    ...
                    continue
                # if i < len(even_pre):
                #     r += (max_odd + even_pre[i],)
                # else:
                #     diff = i - len(even_pre)
                #     if diff % 2 < 1:
                #         diff += 1
                #     r += ((max_odd + even_pre[-1] - even_pre[diff]),)
                odd_needed = max(1, i - len_even)
                if odd_needed % 2 == 0:
                    odd_needed += 1
                if odd_needed > len_odd:
                    r += (0,)
                    continue
                even_needed = i - odd_needed
                if odd_needed > i:
                    r += (0,)
                    continue
                # debug(odd_needed, even_needed)
                r += (max_odd + even_pre[even_needed],)
            p(*r)
