...
if first_input == "first" or 1:
    for _ in range(
        ...
        # if t == 75:
        # print(n, *a, sep="\n")
        b = [[[-1, -1] for _ in range(n)] for _ in range(2)]
        b[0][0] = [a[0][0], a[0][0]]
        for i in range(2):
            for j in range(n):
                cells_before = []
                if i:
                    cells_before.append(b[i - 1][j])
                if j:
                    cells_before.append(b[i][j - 1])
                b[i][j] = [
                    min(
                        max((e[0] for e in cells_before), default=a[i][j]),
                        a[i][j],
                    ),
                    max(
                        min((e[1] for e in cells_before), default=a[i][j]),
                        a[i][j],
                    ),
                ]
        last_cell = b[-1][-1]
        for e in b:
            print(e)
        print(last_cell[0] * (2 * n + 1 - last_cell[1]))
