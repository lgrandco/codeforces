...
def solve():
    *_, n, h, k = map(int, input().split())
    a = list(map(int, input().split()))
    su = sum(a)
    # times_before = max(0, h // su)
    times_before = max(0, -(-h // su) - 1)
    remaining = h - times_before * su
    min_before = [inf]
    max_after = [-inf] * (n + 1)
    for i in range(n):
        min_before.append(min(min_before[-1], a[i]))
        max_after[-i - 2] = max(a[-i - 1], max_after[-i - 1])
    # debug(min_before)
    # debug(max_after)
    cur = 0
    for i, e in enumerate(a):
        cur += e
        # debug(cur, min_before[i + 1], max_after[i])
        if (
            cur >= remaining
            or cur - min_before[i + 1] + max_after[i + 1] >= remaining
        ):
            p(times_before * (n + k) + (i + 1))
            break
    # a = input()
