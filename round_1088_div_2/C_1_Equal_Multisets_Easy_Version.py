...
def solve():
    *_, n, k = map(int, input().split())
    ...
    for i in range(n):
        if b[i] != -1:
            if (
                # abs(i - max(d[b[i]] - k, 0)) >= k
                # or abs(i - min(d[b[i]] + k, n - 1)) >= k
                # or abs(d[b[i]] - max(0, i - k)) >= k
                # or abs(d[b[i]] - min(n - 1, i + k)) >= k
                # min(i, n - k) <= j <= max(0, i - k + 1) + k - 1
                max(0, i - k + 1) != max(0, d[b[i]] - k + 1)
                or min(n - 1, i + k - 1) != min(n - 1, d[b[i]] + k - 1)
            ):
                f = 0
            # debug(
            #     abs(i - max(d[b[i]] - k, 0)), abs(i - min(d[b[i]] + k, n - 1))
            # )
    if all(v == 1 for k, v in Counter(b).items() if k != -1) and (
        n == k
        and set(a) == set(b)
        # or (abs(last) + 1 <= k and (n - first) <= k)
        or f
        # or all(b[i] == a[i] for i in range(n) if b[i] != -1)
    ):
        p("YES")
    else:
        p("NO")
