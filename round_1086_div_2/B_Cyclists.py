...
def solve():
    *_, n, k, p, m = map(int, input().split())
    a = list(map(int, input().split()))
    waiting = deque((e, i) for i, e in enumerate(a, 1))
    heap = []
    r = 0
    cur = 0
    while cur < m:
        popped, idx = waiting.popleft()
        if idx == p:
            if cur + popped > m:
                break
            r += 1
            cur += popped
            waiting.append((popped, idx))
        else:
            heappush(heap, (popped, idx))

        if len(heap) >= k:
            val, idx = heappop(heap)
            cur += val
            waiting.append((val, idx))
    print(r)
