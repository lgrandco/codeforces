...
def solve():
    ...
    #         if j - sum(int(e) for e in str(j)) == i:
    #             l.append(j)
    #     if i % 9 < 1 and not l:
    #         p(i, i / 9, "zzzzzzzzzzzzzzz")
    #     elif i % 9 < 1:
    #         p(i, l)
    # p(i, l)
    a = -(-n // 100) * 100
    a = a - sum(int(e) for e in str(a))
    b = (-(-n // 100) * 100) - 1
    b = b - sum(int(e) for e in str(b))
    # p(a, b)
    p(10 if n % 9 < 1 and not (a > n and b < n) else 0)
